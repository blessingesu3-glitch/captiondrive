...
export const SmartSearchModal: React.FC<SmartSearchModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#111111]/30 backdrop-blur-xs animate-fadeIn select-none">
      <div 
        className="relative w-full max-w-xl bg-white rounded-3xl border border-[#E2E6EC] shadow-premium overflow-hidden p-6 space-y-5"
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#EDEDF8] text-[#14137B] flex items-center justify-center shadow-sm">
        ...
        <div className="relative">
          <input
            ...
            className="w-full h-11 pl-4 pr-24 text-xs sm:text-sm rounded-xl bg-[#F5F7FA]/60 border border-[#E2E6EC] text-[#111111] placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-[#14137B] focus:border-[#14137B]"
          />
          <button
            onClick={() => handleRunSearch()}
            disabled={isSearching || !query.trim()}
            className="absolute right-1.5 top-1/2 -translate-y-1/2 h-8 px-4 rounded-lg bg-[#14137B] hover:bg-[#0E0D57] text-white text-xs font-bold shadow-sm transition-all disabled:opacity-50 cursor-pointer border-none"
          >
          ...
          <div className="flex flex-wrap gap-1.5">
            {samplePrompts.map((p) => (
              <button
                ...
                className="px-2.5 py-1 rounded-lg text-xs bg-[#F5F7FA]/60 hover:bg-[#EDEDF8] hover:text-[#14137B] text-gray-700 border border-[#E2E6EC] hover:border-[#C9C8E8] transition-colors cursor-pointer"
              >
                "{p}"
              </button>
            ))}
        ...
        {/* Results output */}
        {hasSearched && (
          <div className="pt-3 border-t border-[#E2E6EC] space-y-3">
            <div className="flex items-center justify-between text-xs font-bold text-gray-700">
              <span>Results ({matchedResults.length} found)</span>
              {matchedResults.length > 0 && (
                <button
                  onClick={handleApplyResults}
                  className="text-[#14137B] hover:underline flex items-center gap-1 font-bold cursor-pointer border-none bg-transparent"
                >
            ...
            {matchedResults.length > 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 max-h-[220px] overflow-y-auto p-1 custom-scrollbar">
                {matchedResults.map((m) => (
                  <div 
                    key={m.id}
                    className="group relative rounded-xl overflow-hidden bg-[#F5F7FA] border border-[#E2E6EC] aspect-square"
                  >
